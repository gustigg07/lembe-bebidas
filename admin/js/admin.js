...
let cajaDescuentos = new Set(); // porcentajes seleccionados (pueden sumarse: 5+10=15%)
...
function renderPosCart() {
  ...
  // Sumamos todos los porcentajes seleccionados (ej: 5 + 10 = 15%)
...
// Activa/desactiva un porcentaje. Si ya está seleccionado, lo quita. Si no, lo suma.
...
function resetDescuentos() {
  ...
  [5, 10, 15, 20, 25].forEach(v => document.getElementById('desc-' + v)?.classList.remove('sel'));
...
async function cobrar() {
  ...
  // 1. Objeto de venta con descuento detallado
  const venta = {
  ...
  // 2. Registrar venta y descontar stock
  ...
  if (res.ok) {
    ...
    // 3. Ticket y actualización visual
